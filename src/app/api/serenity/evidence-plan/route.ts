...
import { buildSerenityEvidenceExecutionPlan } from "@/lib/serenity/evidencePlan";
...
export async function GET(request: Request) {
  try {
    ...
    const data = buildSerenityEvidenceExecutionPlan({
      ...
      canAutomate: parseBoolean(url.searchParams.get("canAutomate"))
    });
    ...
    return NextResponse.json(
      {
        ...
        data: null,
        error: {
          code: "SERENITY_EVIDENCE_PLAN_FAILED",
          message: error instanceof Error ? error.message : "瓶颈研究证据执行预览失败"
        }
      },
