...
如果候选股 dataCompleteness.level/coreMarketLevel 不是 complete，或缺少 K 线、技术指标、资金流、板块证据中的任一核心数据，不得输出明确买入建议，只能输出：观察、等待回踩、不追、回避、数据不足或减仓。
...
如果 dataCompleteness.companyKnowledgeLevel 不是 sufficient，或公司基础信息/财务/股东数据缺失，不得输出长期持有、长期加仓或长期投资理由；这不等同于核心行情数据不足。
...
7a. action=“数据不足”仅用于核心行情证据缺失或板块证据缺失；若核心行情数据完整但公司认知/财务资料不足，应改用“观察”“等待回踩”“不追”或“回避”，并提示只能按短线主线/题材观察，不能写长期逻辑。
...
12a. 如果 FactPackage.premarket 存在，marketJudgement.logic 和 notifications 应说明盘前外围温度是否只作为背景风险、是否在盘前/竞价阶段压制进攻，以及开盘后需要验证哪些 A 股承接条件；若需要引用盘前判断，必须从证据白名单中选择真实存在的对应 factId，不得使用示例 ID 或自造 ID。
...
21. 必须结合 FactPackage.session.phaseLabel 和 analysisMode 调整输出：盘前强调“今日验证清单”，集合竞价强调“竞价仅弱参考”，午间强调“上午验证与下午条件”，尾盘强调“收盘确认/隔日风险”，收盘后强调“复盘与次日验证”，夜间/非交易日强调“研究计划”。如果引用时段判断，必须从证据白名单中选择真实存在的对应 factId；如果白名单没有时段 factId，只能在文字中结合 session 表达，不得把字段名当作 evidenceRefs。
...
如果无法修复，请根据原因降级：核心行情、K线、技术、资金或板块证据缺失才用“数据不足”；核心行情数据完整但公司认知不足、风险高、买点差或主线匹配弱时用“回避”“不追”或“观察”。
...
export function buildReportPrompt(factPackage: FactPackage | string): string {
  ...
  const evidenceDirective =
    typeof factPackage === "string"
      ? "证据引用白名单：只能引用输入 FactPackage 中真实存在的 factId 或候选股 evidenceRefs；不要引用 JSON 路径、字段名、示例 ID 或白名单外证据。"
      : buildEvidenceDirective(factPackage);
...
function buildRepairContext(factPackage: FactPackage) {
  const evidenceWhitelist = buildEvidenceWhitelist(factPackage);
...
function buildEvidenceDirective(factPackage: FactPackage) {
  const evidenceWhitelist = buildEvidenceWhitelist(factPackage);
  const specialRefs = ["session.market.phase", "premarket.risk.overlay", "audit.constraints", "audit.dataSource.warnings", "audit.dataSource.status"]
    .filter((ref) => evidenceWhitelist.includes(ref));
  return [
    "证据引用白名单：",
    "- 所有 evidenceRefs 只能从 allowedEvidenceRefs 中选择；不要引用 JSON 路径、字段名、示例 ID 或白名单外证据。",
    "- 如果某类证据在白名单中不存在，可以在文字中说明输入缺少该证据，但不得把该缺失 ID 写进 evidenceRefs。",
    specialRefs.length ? `- 当前可用专项证据：${specialRefs.join("；")}` : "- 当前没有可引用的专项证据 ID；盘前/时段/约束类判断只能引用其他真实证据或不写入 evidenceRefs。",
    `- allowedEvidenceRefs: ${JSON.stringify(evidenceWhitelist)}`
  ].join("\n");
}

function buildEvidenceWhitelist(factPackage: FactPackage) {
